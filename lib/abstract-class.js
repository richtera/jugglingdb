...
/** Normalize the query
  ...
  *  limit: Number
  *  skip: Number
  * if neither where, order, limit or skip exist then we'll assume this is just
  * a query (i.e. where) and will then promote it to where: where
  *
  * in mongoose compatibility mode:
  *  same as above except that the keys are $where, $order (or $sort), $limit and $skip
  */
AbstractClass.unnormalizeQuery = function (query) {
  if (!query) {
    return {};
  }
  if (schema.Schema.mongooseMode) {
    var q = {};
    if ((query.where !== undefined) || (query.limit !== undefined) || (query.order !== undefined)
    || (query.skip !== undefined)) {
      ...
      if (query.sort || query.order)
        q.$order = query.sort || query.order;
      if (query.skip)
        q.$skip = query.skip;
    } else {
      q = {$where:query};
...
AbstractClass.normalizeQuery = function (query) {
  if (!query) {
    return {};
  }
  var q;
  if (schema.Schema.mongooseMode) {
    if ((query.$where !== undefined) || (query.$limit !== undefined)
    || (query.$order !== undefined) || (query.$sort !== undefined) || (query.$skip !== undefined)) {
      q = {};
      if (query.$where) {
        q.where = query.$where;
      } else {
...
AbstractClass.findOne = function findOne(params, cb) {
    ...
    params.limit = 1;
    this.all(this.unnormalizeQuery(params), function (err, collection) {
...
AbstractClass.count = function (where, cb) {
    ...
    if (typeof where === 'function') {
        ...
        where = this.normalizeQuery(where).where;
    }
...
function defineScope(cls, targetClass, name, params, methods) {
            ...
            var f = function caller(condOrRefresh, cb) {
                ...
                if (arguments.length === 1) {
                    ...
                    if (typeof condOrRefresh === 'boolean') {
                        ...
                        actualCond = targetClass.normalizeQuery(condOrRefresh);
                        actualRefresh = true;
                        saveOnCache = false;
                    }
                } else {
                ...
                if (!this.__cachedRelations || (typeof this.__cachedRelations[name] == 'undefined') || actualRefresh) {
                    var self = this;
                    return targetClass.all(targetClass.unnormalizeQuery(mergeParams(actualCond, caller._scope)), function(err, data) {
                        if (!err && saveOnCache) {
    ...
    function build(data) {
        return new targetClass(mergeParams(this._scope || {where:{}}, {where:data}).where);
    }
    ...
    */
    function destroyAll(cb) {
        targetClass.all(targetClass.unnormalizeQuery(this._scope), function (err, data) {
            if (err) {
                cb(err);
            } else {
